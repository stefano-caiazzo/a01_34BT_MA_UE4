...
#if WITH_EDITOR
const float UAkPortalComponent::RoomsRefreshIntervalEditor = 0.1f;
#endif
	...
	Super(ObjectInitializer)
{
	...
	OpenPortalCounter = (int)InitialState;
	...
	if (world != nullptr)
	{
		...
		if ( ! (worldType == EWorldType::Game || worldType == EWorldType::PIE))
			RoomsRefreshIntervalSeconds = RoomsRefreshIntervalEditor;
	}
#endif
}
...
{
	return OpenPortalCounter > 0 ? AkAcousticPortalState::Open : AkAcousticPortalState::Closed;
}
...
{
	/* While the component is moving (or being moved in the editor), update the room connections (at a rate of 1/RoomsRefreshIntervalSeconds times a second). */
	if (RoomConnectionsNeedUpdated && (GetWorld()->TimeSeconds - LastRoomsUpdate) > RoomsRefreshIntervalSeconds)
	{
		const bool RoomsChanged = UpdateConnectedRooms();
		if (RoomsChanged)
		{
...
void AAkAcousticPortal::FitRaycast()
{
	...
	for (int i = 0; i < hits.Num() && !bIntersects; ++i)
	{
		...
		for (int j = i + 1; j < hits.Num() && !bIntersects; ++j)
		{
			...
			if (fabsf(FVector::DotProduct(diri, dirj)) < kDotEpsilon)
			{
