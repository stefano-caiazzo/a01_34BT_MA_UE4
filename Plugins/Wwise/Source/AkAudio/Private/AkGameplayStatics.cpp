		...
		if (ExternalSources.Num() > 0)
		{
			...
			if (PlayingID != AK_INVALID_PLAYING_ID)
			{
				for (auto ExtSrc : ExternalSources)
				{
					...
					{
						ExtSrc.ExternalSourceAsset->AddPlayingID(DeviceAndWorld.AkAudioDevice->GetIDFromString(GET_AK_EVENT_NAME(AkEvent, EventName)), PlayingID);
	...
	FLatentActionInfo LatentInfo)
{
	...
	if (DeviceAndWorld.IsValid())
	{
		...
		FWaitEndOfEventAction* NewAction = LatentActionManager.FindExistingAction<FWaitEndOfEventAction>(LatentInfo.CallbackTarget, LatentInfo.UUID);
		if (!NewAction)
		{
